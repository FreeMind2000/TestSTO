...
//---------------------------------------------------------------------------
// Версия: 1.03
// Дата: 14 января 2013г
// - поменял формулы преобразования координат на прямое преобразование Лоренца по dt
// - добавил еще 3 эксперимента
//   - Относительность одновременности2
//   - Часы Эйнштейна
//   - Решающий эксперимент (Последний довод Короля: "Часы д'Артаньяна")
// - добавлено описание в хелп
//---------------------------------------------------------------------------
...
public:
	...
	// тип заголовка
	static const int t_ct_none = 0;		// никакой информации в заголовке - невидимый
	static const int t_ct_full = 0xFFFF;// полная информация об объекте
	static const int t_ct_name = 1;		// только имя
	static const int t_ct_name_time = 2;// только имя + время
	int caption_type;
...
public:
	...
	MMatObject
	(
	) : caption_color((TColor)RGB(0,255,100)), caption_type(t_ct_full), caption_up(true)
	{}
...
//---------------------------------------------------------------------------
// Класс для датчика - зеркала
//---------------------------------------------------------------------------
// Логика работы:
// - Датчик срабатывает на объекты определенного типа
// (тип определяется по 1й букве в имени объекта "F" - фотон...)
// - Начально состояние датчика (счетчика касаний) = 0
// - Если в текущий момент времени в точке в которой находится датчик
// (относительно ИСО с которой связан датчик)
// одновременно находится хотя бы 1 объект или больше
// (т.е. имеют координаты == координате точки в которой находится датчик)
// то - датчик срабатывает-включается (состояние = счетчик = увеличивается +1)
// - Если в текущий момент времени в точке в которой находится датчик
// находится 0 объектов состояние датчика не меняется (состояние = старое_состояние)
//---------------------------------------------------------------------------
class MSensorMirror : public MSensorBase
{
public:
	// Тип датчика определяется по 1й букве в имени объекта "F" - фотон...
	AnsiString type;
	...
	// счетчик касаний
	static const int t_sost_undefined = 0;
	static const int t_sost_no_count = 1000;
	int sost;
...
//---------------------------------------------------------------------------
// Класс для датчика - стрелки часов
//---------------------------------------------------------------------------
// Логика работы:
// - Датчик срабатывает на объекты определенного типа
// (тип определяется по 1й букве в имени объекта "d" - фотон...)
// - Начально состояние датчика (счетчика касаний) = 0
// - Если в текущий момент времени в точке в которой находится датчик
// (относительно ИСО с которой связан датчик)
// одновременно находится хотя бы 1 объект или больше
...
// находится 0 объектов состояние датчика не меняется (состояние = старое_состояние)
//---------------------------------------------------------------------------
class MSensorStrelka : public MSensorBase
{
public:
	// Тип датчика определяется по 1й букве в имени объекта "F" - фотон...
	AnsiString type;
	...
	// состояние только что было изменено
	bool sost_update;
...
public:
	...
	MExperimentBase
	(
	) : abs_iso(NULL), move_iso(NULL), m_frmHelp(NULL)
	{}
...
//---------------------------------------------------------------------------
// Класс для эксперимента LastArgumentOfKing2
//---------------------------------------------------------------------------
class MExperimentLastArgumentOfKing2 : public MExperimentBase
{
public:
	//---------------------------------------------------------------------------
	// Инициализация
...
//---------------------------------------------------------------------------
// Класс для эксперимента MExperimentEnshtainTimers
//---------------------------------------------------------------------------
class MExperimentEnshtainTimers : public MExperimentBase
{
public:
	//---------------------------------------------------------------------------
	// Инициализация
...
//---------------------------------------------------------------------------
// Класс для эксперимента MExperimentKingTimers
//---------------------------------------------------------------------------
class MExperimentKingTimers : public MExperimentBase
{
public:
	//---------------------------------------------------------------------------
	// Инициализация
